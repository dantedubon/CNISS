...
namespace CNISS_Integration_Test.Repositories.EmpleoRepository.Commands
{
    ...
    public class when_UseAddFichaSupervisionFromMovilInformation_Should_UpdateEmpleo
    {
        ...
        private Because of = () =>
        {
            ...
            var fichaSupervision = new FichaSupervisionEmpleo(supervisor, firmaAutorizada, "posicion GPS", "cargo",
                "funciones", "31000000", "31000000", 9, new ContentFile(new byte[] {0, 1}));
           

            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                var repository = new EmpleoRepositoryCommands(uow.Session);
                repository.updateFromMovilVisitaSupervision(_expectedEmpleo.Id,fichaSupervision);
                uow.commit();
            }

           
            _expectedEmpleo.supervisado = true;
            _expectedEmpleo.addFichaSupervision(fichaSupervision);
        };
