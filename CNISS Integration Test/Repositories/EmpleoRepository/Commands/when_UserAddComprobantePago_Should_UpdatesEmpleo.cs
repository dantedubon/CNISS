...
namespace CNISS_Integration_Test.Repositories.EmpleoRepository.Commands
{
    ...
    public class when_UserAddComprobantePago_Should_UpdatesEmpleo
    {
        ...
        private static ComprobantePago _comprobantePagoNuevo;
        ...
        private Because of = () =>
        {
           _expectedEmpleo.addComprobante(_comprobantePagoNuevo);
            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                var repository = new EmpleoRepositoryCommands(uow.Session);
        ...
        private static ComprobantePago getComprobantePago2()
        {
            return new ComprobantePago(new DateTime(2014, 8, 16), 100, 200, 100);
        }
