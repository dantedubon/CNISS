...
namespace CNISS_Integration_Test.Repositories.EmpleoRepository.Commands
{
    ...
    public class when_UserRemoveComprobantePago_Should_UpdatesEmpleo
    {
        ...
        private static ComprobantePago _comprobantePagoToRemove;
        ...
        private Because of = () =>
        {
            _expectedEmpleo.removeComprobante(_comprobantePagoToRemove);
            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                var repository = new EmpleoRepositoryCommands(uow.Session);
