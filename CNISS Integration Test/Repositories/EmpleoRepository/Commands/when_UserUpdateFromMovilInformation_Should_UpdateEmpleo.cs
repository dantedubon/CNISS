...
namespace CNISS_Integration_Test.Repositories.EmpleoRepository.Commands
{
     [Subject(typeof (EmpleoRepositoryCommands))]
        public class when_UserUpdateNotaDespidoFromMovilInformation_Should_UpdateEmpleo
        {
            ...
            private Because of = () =>
            {
                var motivoDespido = new MotivoDespido("Impuntual");
                var userSupervisor = new User("supervisor", "supervisor1", "", "xxx", "xxx",
                    new Rol("RolSupervisor", "RolSupervisor"));
                var userFima = new User("firma", "firma", "", "xxx", "xxx", new Rol("Firma", "Firma"));
                var firmaAutorizada = new FirmaAutorizada(userFima, new DateTime(2014, 8, 1));
                var supervisor = new Supervisor(userSupervisor);

                prepareUser(userSupervisor);
                prepareUser(userFima);
                prepareMotivoDespido(motivoDespido);
                prepareFirmaAutorizada(firmaAutorizada);
                prepareSupervisor(supervisor);

                var notaDespido = new NotaDespido(motivoDespido, new DateTime(2014, 5, 1), "posicion",supervisor
                    , firmaAutorizada)
                {
                    documentoDespido = new ContentFile(new byte[] {1, 1})
                };

                using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
                {
                    var repository = new EmpleoRepositoryCommands(uow.Session);
                    repository.updateFromMovilNotaDespido(_expectedEmpleo.Id,notaDespido);
                    uow.commit();
                }

                _expectedEmpleo.notaDespido = notaDespido;
                _expectedEmpleo.supervisado = true;

            };

            private It should_save_empleo = () =>
            {
            ...
            private static void prepareBeneficiario(Beneficiario beneficiario)
            {
                ...
                beneficiario.addDependiente(getDependiente(new Identidad("0801196712396"),
                    new Nombre("Lavinia", "Dubon", "Fajardo"), parentescoMadre));
                beneficiario.addDependiente(getDependiente(new Identidad("0801196712395"),
         ...
         private static void prepareMotivoDespido(MotivoDespido motivoDespido)
         {
             _session = _sessionFactory.OpenSession();
             using (var tx = _session.BeginTransaction())
             {
                 _session.SaveOrUpdate(motivoDespido);
                 tx.Commit();
             }
             _session.Close();
         }

         private static void prepareSupervisor(Supervisor supervisor)
         {
             _session = _sessionFactory.OpenSession();
             using (var tx = _session.BeginTransaction())
             {
                 _session.SaveOrUpdate(supervisor);
                 tx.Commit();
             }
             _session.Close();
         }
         private static void prepareFirmaAutorizada(FirmaAutorizada firmaAutorizada)
         {
             _session = _sessionFactory.OpenSession();
             using (var tx = _session.BeginTransaction())
             {
                 _session.SaveOrUpdate(firmaAutorizada);
                 tx.Commit();
             }
             _session.Close();
         }
            ...
            private static IList<Sucursal> getSucursales()
            {
                ...
                var firma1 = new FirmaAutorizada(
                    new User("DRCD", "Dante", "Ruben", "SDSD", "as", new Rol("rol", "rol")), fechaDeCreacionFirma);
                var firma2 =
                    new FirmaAutorizada(new User("Angela", "Angela", "Castillo", "SSS", "SS", new Rol("rol", "rol")),
                        fechaDeCreacionFirma);
