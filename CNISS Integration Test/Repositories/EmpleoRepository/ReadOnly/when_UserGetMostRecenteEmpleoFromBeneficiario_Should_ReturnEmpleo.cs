...
namespace CNISS_Integration_Test.Repositories.EmpleoRepository.ReadOnly
{
    ...
    public class when_UserGetMostRecenteEmpleoFromBeneficiario_Should_ReturnEmpleo
    {
        private static InFileDataBaseTest _dataBaseTest;
        private static ISessionFactory _sessionFactory;
        private static ISession _session;
        private static Empleo _empleoMasReciente;
        private static Empleo _empleoAnterior;
        private static Empleo _responseEmpleo;
        private static bool _response;
        private static Identidad _identidadBeneficiario;
        ...
        private Establish context = () =>
        {
            ...
            _identidadBeneficiario = beneficiario.Id;
            ...
            var fechaDeInicio = DateTime.Now.AddMonths(-2).Date;

            _empleoMasReciente = new Empleo(empresa, sucursal, beneficiario, horario, "Ingeniero", 25000, tipoEmpleo,
                fechaDeInicio);
            _empleoMasReciente.addComprobante(getComprobantePago());

            var fechaDeInicioEmpleoAnterior = DateTime.Now.AddMonths(-6).Date;
            _empleoAnterior = new Empleo(empresa, sucursal, beneficiario, horario, "Ingeniero", 25000, tipoEmpleo,
                fechaDeInicioEmpleoAnterior);
        ...
        private Because of = () =>
        {
            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                ...
                repository.save(_empleoMasReciente);
                repository.save(_empleoAnterior);
         ...
         It should_return_empleo = () =>
        {
            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                var repositoryRead = new EmpleoRepositoryReadOnly(uow.Session);


                _responseEmpleo = repositoryRead.getEmpleoMasRecienteBeneficiario(_identidadBeneficiario);
                _responseEmpleo.Empresa.Gremial.Empresas = new List<Empresa>();
                _responseEmpleo.ShouldBeEquivalentTo(_empleoMasReciente);
            }
        ...
        private static IList<Sucursal> getSucursales()
        {
            ...
            var firma1 = new FirmaAutorizada(new User("DRCD", "Dante", "Ruben", "SDSD", "as", new Rol("rol", "rol")),
