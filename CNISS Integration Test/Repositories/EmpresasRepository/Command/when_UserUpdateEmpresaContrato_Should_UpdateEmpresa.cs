...
namespace CNISS_Integration_Test.Repositories.EmpresasRepository.Command
{
    ...
    public class when_UserUpdateEmpresaContrato_Should_UpdateEmpresa
    {
        ...
        private Because of = () =>
        {
            var nuevaData = new byte[] {1, 1, 1};
            var nuevoContrato = new ContentFile(nuevaData);
            _expectedEmpresa.Contrato = nuevoContrato;
            using (var uow = new NHibernateUnitOfWork(_sessionFactory.OpenSession()))
            {
                var gremioRepository = new GremioRepositoryReadOnly(uow.Session);
                _repositoryCommands = new EmpresaRepositoryCommands(uow.Session, gremioRepository);
                _repositoryCommands.updateContrato(_expectedEmpresa.Id,nuevoContrato);
                uow.commit();
            }
        ...
        It should_update_empresa = () =>
        {
            ...
            using (var tx = _session.BeginTransaction())
            {
                ...
                _responseEmpresa.Gremial.Empresas = new List<Empresa>();
