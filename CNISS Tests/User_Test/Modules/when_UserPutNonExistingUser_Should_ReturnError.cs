...
namespace CNISS_Tests.User_Test.Modules
{
    ...
    public class when_UserPutNonExistingUser_Should_ReturnError
    {
         ...
         Establish context = () =>
        {
            ...
            _userRequest.userRol = Builder<RolRequest>.CreateNew().Build();

            _commandUpdate = Mock.Of<ICommandUpdateIdentity<User>>();
            _repositoryRead = Mock.Of<IUserRepositoryReadOnly>();
            Mock.Get(_repositoryRead).Setup(x => x.exists(Moq.It.IsAny<string>())).Returns(false);
